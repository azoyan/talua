...
describe("#min Minimum", function()
    local Minimum = require "Minimum"
    local Bar = require "Candlestick"

    it("add() last() #min1", function()
        local min = Minimum(14)

        -- data for the test is taken from here http://investexcell.net
        local input = min:add{Bar():high(67.27):low(65.75):close(65.98), Bar():high(65.7):low(65.04):close(65.11),
        ...
        assert.are.approx_same(58.8, input:last(), 0.1)
    end)

    it("add(), series()", function()
        local min = Minimum(4)
        min:add{Bar():high(67.27):low(65.75):close(65.98), Bar():high(65.7):low(65.04):close(65.11),
                              Bar():high(65.05):low(64.26):close(64.97), Bar():high(65.16):low(64.09):close(64.29),
                              Bar():high(62.73):low(61.85):close(62.44), Bar():high(62.02):low(61.29):close(61.47)}

        local expected = {64.97, 64.29, 62.44, 61.47}
        assert.are.approx_same_series(expected, min:series())
    end)

    it("add(), last(), reset()", function()
        local min = Minimum(4)
        ...
        assert.are.approx_same_series(expected, min:series())
        min:reset()
        assert.are.falsy(min:last())
        assert.are.same(0, #min:series())
