...
describe("#atr Average True Range", function()
    ...
    it("add() series() #atr1", function()
        local atr = AverageTrueRange(14)
        ...
        local series = atr:series()
        assert.are.approx_same_series(expected, series, 0.2)
        atr:reset()
        assert.are.same(0, #atr:series())
        assert.falsy(atr:last())
        assert.are.approx_same_series(expected, atr:add(input):series(), 0.2)
    end)
end)
