...
describe("#max Maximum", function()
    local Maximum = require "Maximum"
    local Bar = require "Candlestick"

    it("add() series() #max1", function()
        local max = Maximum(14)

        -- data for the test is taken from here http://investexcell.net
        local max = max:add{Bar():high(67.27):low(65.75):close(65.98), Bar():high(65.7):low(65.04):close(65.11),
        ...
        local series = max:series()
        assert.are.approx_same(69.01, max:last(), 0.1)
    end)
    ...
    it("add(), series()", function()
        ...
        local expected = {64.09, 64.97, 65.11, 65.98}
        max:add(input)
        assert.are.approx_same_series(expected, max:series())
    end)
    ...
    it("add(), last(), reset()", function()
        ...
        assert.are.approx_same_series(expected, max:series())
        max:reset()
        assert.are.falsy(max:last())
        assert.are.same(0, #max:series())
